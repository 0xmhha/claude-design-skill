...
**Active version**: **v1.0.0 released 2026-05-12** · Step 1–7 shipped (2026-05-10 → 2026-05-12) + `/buddy:plan-build` invocation (2026-05-12) producing 9 plan documents under `docs/plan/` + `/buddy:autoplan` audit trail recorded in `docs/plan/00-overview.md §8`. v1.0.0 ship sealed by `/buddy:ship-release` D-01~D-06 sequence (CHANGELOG split · plugin/marketplace version bump · doc bumps · git tag v1.0.0 · GitHub release · marketplace source.ref pinned to v1.0.0). Next phase: §5 build-feature (A category dogfooding) → §8 iterate-product (backlog from dogfood friction). 108 regression tests unchanged. Start at `docs/plan/00-overview.md`.
...
- **7.2** — ✅ shipped: `.claude-plugin/plugin.json` (root) declares the skill as a Claude Code plugin (name `claude-design-skill` · version `1.0.0` · Apache-2.0 · homepage + repository + 10 keywords). `.claude-plugin/marketplace.json` (root) declares a single-plugin marketplace pointing at `.` so the repo *itself* is a marketplace — teams add it once via `claude plugin marketplace add <git-url>` and any teammate installs via `claude plugin install claude-design-skill`. README directory tree gains the `.claude-plugin/` row.
