...
- `scripts/figma-to-brand-spec.py` (~280 lines, stdlib only) — Figma → `team-brand-spec.json` extractor. Reads a Figma file via the public REST API (`GET /v1/files/{key}`, `X-Figma-Token` header), walks the document tree, resolves nodes that reference named styles, and maps them to spec slots via a slash naming convention (`color/<group>/<token>` → `colors.<group>.<token>`, `text/<role>/family` → `typography.<role>.family`, etc.). FILL solid colours round Figma's 0-1 RGB to 0-255 HEX; TEXT styles capture family / weight / size / line-height. Unknown style names land in `_meta.unmapped_styles` (never silently dropped). Merges into `team-brand-spec.default.json` by default so untouched groups keep their evidence-anchored values; `--no-merge` and `--base <path>` opt out.
- `scripts/fixtures/figma_minimal.json` — synthetic Figma API response (6 named styles + 1 deliberate-mismatch + 1 unstyled black rect) that the test suite exercises so CI runs without network or tokens.
- `scripts/test_figma_to_brand_spec.py` — 13 regression tests covering happy paths (colour + text mapping, hex rounding accuracy, surface / accent / status tokens), unmapped-style preservation, unstyled-node leakage guard, merge / no-merge modes, every error path (missing token, missing fixture, invalid JSON, output collision), `--force` overwrite, and round-trip JSON validity.
- `references/figma-to-brand-spec.md` — 8-section reference: extraction model · Figma naming convention · usage table · reference public Figma files (Material 3 Design Kit + iOS 18 community kit) · output shape with provenance · out-of-scope boundaries (no round-trip, no CSS scraping, no image extraction, no auto-reextraction) · license posture.
- `.github/workflows/sanitizers.yml` gains the `figma-to-brand-spec extractor tests` step; the workflow now runs **93 tests** total (18 + 13 + 19 + 19 + 11 + 13). SKILL.md routing table gains rows for `web3-game-style-stats.md` and `figma-to-brand-spec.md` so future agents reach them directly. SKILL.md / README / HANDOFF / PROJECT-PLAN status headers all bumped from "Step 1–4 shipped" to "Step 1–5 shipped (2026-05-10 → 2026-05-11)".
...
### Added — Repository governance docs and Dependabot

- `SECURITY.md` — vulnerability disclosure policy. Names the GitHub private-security-advisory channel as the preferred report path, scopes in (sanitizer / scanner / import-gate / codename catalog / workflow / docs / settings template) vs out (third-party services referenced via the allowlist, the upstream `alchaincyf/huashu-design`, fork-specific brand customization), states the acknowledgement-within-7-days guidance, and lists the *not a vulnerability* cases (the conservative-pairing trade-off in the codename catalog, `prefers-reduced-motion` enforcement boundaries, anything in the upstream).
- `CONTRIBUTING.md` — three audience paths: fork operators (with the Step 4 ⏳ checklist pointer), security reporters (pointer to SECURITY.md and the private advisory flow), and rare-PR contributors (must pass the 78-test workflow, must keep the license-clean read discipline, must keep the animation engine API-compatible). Records the project conventions for commit messages, the absence of AI co-authorship trailers, and the append-only decisions log.
- `.github/dependabot.yml` — Dependabot config for the `github-actions` ecosystem only (the repo has no language manifests; the only pinned dependency surface is `actions/checkout@v4` / `setup-python@v5` / `setup-node@v4` in the workflow). Weekly schedule, Monday 09:00 Asia/Seoul, max 3 open PRs, grouped under `actions/*` so a major-version bump in one of them lands as one PR not three.
- README.md gets matching pointers — the Contributing section now points at `CONTRIBUTING.md` and `SECURITY.md`; the directory tree adds the four new files.

### Changed — License: MIT → Apache 2.0

- `LICENSE` replaced with the Apache License 2.0 standard text plus an appendix-style maintainer note explaining the change. `NOTICE` added (required by Apache §4(d)) — names the clean-room rewrite status, points at `PROJECT-PLAN.md §2` (23 carry-over files inventory) and `PROJECT-PLAN.md §7` (verbatim sections inside `references/design-styles.md §15–18` and `references/scene-templates.md §06–09`), and clarifies that pinned-version third-party content (Lucide ISC · Phosphor MIT · MDN CC-BY-SA 2.5 · React / Babel MIT) is referenced via the allowlist but not redistributed by this repository.
- **Rationale**: Apache 2.0 adds an explicit patent grant (§3) and trademark / contributor clarity (§6) that MIT does not. Unrelated to upstream licensing — the upstream `alchaincyf/huashu-design` skill carries a separate Personal-Use license; this repository is a clean-room rewrite that does not derive from it; the upstream license is unaffected by the Apache 2.0 grant recorded here.
- Doc updates: `HANDOFF.md §0` banner + §14 closing list; `README.md` license section + directory-tree comment + new NOTICE entry; `PROJECT-PLAN.md §1` result line; `PROJECT-PLAN.md §7` decisions log gets the rationale + license-clean evidence reaffirmation. Historical Step 1 mentions of "MIT" inside the PROJECT-PLAN decisions log (entries dated 2026-05-09) are left unchanged — the decisions log is append-only.
